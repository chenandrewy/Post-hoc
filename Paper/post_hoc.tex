...
Using theory rules out some ideas, and is characterized by a set 
\begin{align}
...
\begin{itemize}
\item \emph{a priori}: the researcher writes down a theory that implies a selected idea $i^\ast$, which is randomly-selected from $S$. (In this simple model, all ideas are equally consistent with theory.)
...
% tbc: address how \mathcal{O} doesn't mean -no- data
