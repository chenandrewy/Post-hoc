...
\label{eq:unbiased}
\end{align}
[tbc: address small bias at $\hat{\mu}_{i} \approx \text{median}(\mu_i)$ due to $\hat{\mu}_{i}$ not accounting for the theorist's signal] In contrast, the expected $\hat{\mu}_{i}$ from \emph{post hoc} theorizing is biased:
...
Lemmas \ref{lem:ez:biased} and \ref{lem:ez:post-hoc-opt} are illustrated in Figure \ref{fig:ez}. It simulates 100 selected ideas, with $N=20$, $\mu_i \sim \text{Normal}\left(0, 2^2\right)$, and $\varepsilon_i \sim \text{Normal}\left(0, 1\right)$. \emph{A priori} theorizing leads to approximately unbiased estimates, seen in the dots evenly spread across the 45 degree line. However, \emph{post hoc} theorizing leads to higher quality ideas, seen  in how the stars tend to lie toward the top of the chart.
...
\begin{figure}[t]
    \def\tempwidth{0.8\textwidth}

    \centering
    \begin{subfigure}[b]{\textwidth}
        \centering
        \caption{\emph{A priori} Selection}
        \includegraphics[width=\tempwidth]{../Results/litplus-muhat-ap.pdf}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \includegraphics[width=\tempwidth]{../Results/litplus-muhat-ph.pdf}
    \end{subfigure}
